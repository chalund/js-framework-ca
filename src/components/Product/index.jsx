...
function Product ({product: {id, image, title, price, discountedPrice}, onAddToCartClick  }) {
        ...
        <div key={id} className="max-w-sm bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700 m-5 hover:shadow-md transition-transform transform-gpu hover:scale-105">
            ...
            <div className="p-5 flex flex-col" style={{ width: '300px'}}>
                ...
                <div className="flex ">    
                    {discountedPrice && discountedPrice !== price && (
                        <>
                            ...
                            <p className="mb-3 ms-4 font-normal text-red-600">ON SALE ${discountedPrice}</p>
                        </>
                    )}
                    {(!discountedPrice || discountedPrice === price) && (
                ...
                <div className="flex justify-start items-center mt-auto h-full">
                    <button onClick={handleButtonOnClick} className="bg-purple-600 text-white font-bold py-1 px-3 rounded mr-3 hover:bg-orange-400" style={{ height: '2rem' }}><FaShoppingCart /></button>
                    <Link to={`/product/${id}`}>
                        <button className="bg-purple-600 text-white font-bold py-1 px-3 rounded hover:bg-orange-400" style={{ height: '2rem' }}>View Product</button>
                    </Link>
                </div>
            </div>
